...
class Rawat_inapController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->rawat_inap_Tanggal_Masuk)){
			$vals = explode("-to-", str_replace(" ", "", $request->rawat_inap_Tanggal_Masuk));
			$startdate = $vals[0];
			$enddate = $vals[1];
			$db->where("rawat_inap.Tanggal_Masuk BETWEEN '$startdate' AND '$enddate'");
		}
		if(!empty($request->rawat_inap_Tanggal_Keluar)){
			$vals = explode("-to-", str_replace(" ", "", $request->rawat_inap_Tanggal_Keluar));
			$startdate = $vals[0];
			$enddate = $vals[1];
			$db->where("rawat_inap.Tanggal_Keluar BETWEEN '$startdate' AND '$enddate'");
		}
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("No_RM","No_SEP","Nama_Pasien","Tanggal_Masuk","Tanggal_Keluar","Keterangan");
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","No_RM","No_SEP","Nama_Pasien","Tanggal_Masuk","Tanggal_Keluar","Keterangan");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'No_RM' => 'required|numeric',
